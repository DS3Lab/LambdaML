...
class LR_ADMM_W_LOSS(torch.nn.Module):
    def __init__(self, _w, _z, _u):
        super(LR_ADMM_W_LOSS, self).__init__()
        self.linear = torch.nn.Linear(_w.shape[0], _w.shape[1])
        self.linear.weight = _w
        self.z = _z
        self.u = _u
        return
...
def prune_weight(weight, device, percent):
    ...
    mask = torch.Tensor((abs(weight_numpy) >= pcen).astype(int)).to(device)
    return mask
...
def prune_l1_weight(weight, device, delta):
    ...
    weight_numpy[under_threshold] = 0
    mask = torch.Tensor((abs(weight_numpy) >= delta).astype(int)).to(device)
    return mask
