...
for ((i=0; i<$world_size; i++)); do
  ...
  rm -f logs/$log_file
  nohup python higgs_svm_grad_avg.py --init-method tcp://$master_ip --world-size $world_size --rank $i --root /home/ubuntu/dataset/higgs/ --no-cuda --batch-size $batch_size --learning-rate $learning_rate --epochs $epochs > logs/$log_file 2>&1 &
done
