...
then
  ...
  source /home/ubuntu/envs/pytorch/bin/activate
  nohup python3.6 /home/ubuntu/LambdaML/ec2/lr/rcv_lr.py --rank $i --batch-size $batch_size -l $lr --world-size $world_size --train-file "${train_path}/${i}_${world_size}" --no-cuda > "/home/ubuntu/log/lr_local_${i}_t${world_size}_b${batch_size}_lr${lr}.txt" 2>&1 &
done
fi
