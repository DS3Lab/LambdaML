...
# ./run_lr_higgs_local.sh 16 100 6250 0.01 30 172.31.41.172:24000 ~/dataset/higgs/
...
for ((i=0; i<$world_size; i++)); do
  ...
  rm -f ~/logs/$log_file
  nohup python -u higgs_lr_grad_avg.py --init-method tcp://$master_ip --rank $i --world-size $world_size --epochs $epochs --batch-size $batch_size --learning-rate $learning_rate --features $features --root $root --no-cuda > ~/logs/$log_file 2>&1 &
done
