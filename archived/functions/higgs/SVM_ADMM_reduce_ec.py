...
def handler(event, context):
    ...
    for admm_epoch in range(num_admm_epochs):
        ...
        u_w_b_merge = reduce_epoch(u_w_b, merged_bucket, num_workers, worker_index, postfix)

        u_mean = u_w_b_merge[:u_shape[0] * u_shape[1]].reshape(u_shape) / float(num_workers)
        w_mean = u_w_b_merge[u_shape[0]*u_shape[1] : u_shape[0]*u_shape[1]+w_shape[0]*w_shape[1]].reshape(w_shape) \
                 / float(num_workers)
