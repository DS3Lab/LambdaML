...
for ((i=0; i<$world_size; i++)); do
  if [[ $i == 0 ]]
	then
	  source $env_path
	  cd $code_path
	  dir_path="${home_path}/logs/LR_higgs_admm_w${world_size}_lr${lr}_b${batch_size}/"
	  rm -rf $dir_path
	  mkdir -p $dir_path
    log_path=$dir_path$i"_"$world_size".log"
    rm -f $log_path
	  nohup python -u higgs_lr_admm.py --init-method tcp://$master_ip --rank 0 --world-size $world_size \
    ...
    ssh $node_name "source $env_path; cd $code_path; nohup python -u higgs_lr_admm.py --init-method tcp://$master_ip --rank $i --world-size $world_size --admm-epochs $admm_epochs --epochs $epochs --features $n_features --classes $n_classes --learning-rate $lr --batch-size $batch_size --root $root --no-cuda > $log_path 2>&1 &"
  fi
done