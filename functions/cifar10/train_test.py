...
def train(epoch, net, trainloader, optimizer, device, worker_index, num_worker, sync_mode, sync_step):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if num_batch%10 == 0:
            print("Epoch {} Batch {} training Loss:{}, Acc:{}".format(epoch, num_batch, train_loss, train_acc))
        
        num_batch += 1
        
    epoch_time = time.time() - epoch_start
    print("Epoch {} has {} batches, time = {} s, sync time = {} s, cal time = {} s".format(epoch, num_batch, epoch_time, epoch_sync_time, epoch_time - epoch_sync_time))
