...
def handler(event, context):
    ...
    for epoch in range(num_epochs):
        ...
        if worker_index == 0:
            ...
            w_model_merge, b_model_merge = get_merged_w_b_grad(endpoint,model_bucket, file_postfix,
                                                                w_moel.dtype, w_model.shape, b_model.shape,
                                                                w_prefix, b_prefix)

        model.linear.weight = Variable(torch.from_numpy(w_model_merge))
        model.linear.bias = Variable(torch.from_numpy(b_model_merge))
