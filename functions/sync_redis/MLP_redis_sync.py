...
def handler(event, context):
    ...
    for epoch in range(num_epochs):
        for batch_index, (items, labels) in enumerate(train_loader):
            ...
            w_shape = []
            w_grad = [model.fullyconnected1.weight.grad.data.numpy()]
            w_shape.append(w_grad[-1].shape)
            w_grad.append( model.fullyconnected2.weight.grad.data.numpy())
            w_shape.append(w_grad[-1].shape)
            w_grad.append( model.fullyconnected3.weight.grad.data.numpy())
            w_shape.append(w_grad[-1].shape)
            b_shape = []
            b_grad = [model.fullyconnected1.bias.grad.data.numpy()]
            b_shape.append(b_grad[-1].shape)
            b_grad.append( model.fullyconnected2.bias.grad.data.numpy() )
            b_shape.append(b_grad[-1].shape)
            b_grad.append(model.fullyconnected3.bias.grad.data.numpy() )
            b_shape.append(b_grad[-1].shape)            
            print("w_grad before merge = {}".format(w_grad[0][0][0:5]))
            print("b_grad before merge = {}".format(b_grad[0]))
            
            #synchronization starts from that every worker writes their gradients of this batch and epoch
            sync_start = time.time()
            for layer in range(3):
                hset_object(endpoint, grad_bucket, w_grad_prefix + str(layer)+ "_" + str(worker_index), w_grad[layer].tobytes())
                hset_object(endpoint, grad_bucket, b_grad_prefix +str(layer)+ "_" + str(worker_index), b_grad[layer].tobytes())
            print(sizeof(w_grad.tobytes()))
            ...
            if worker_index == 0:
                ...
                w_grad_merge, b_grad_merge = \
                    merge_w_b_grads_layers(endpoint, 
                                    grad_bucket, num_worker, w_grad[0].dtype,
                                    w_shape, b_shape,
