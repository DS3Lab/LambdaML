...
def process_centroid(centroid_byte, num_clusters):
	cent = np.frombuffer(centroid_byte)
	cent_size = cent.shape[0]
	return cent.reshape(int(cent_size/num_clusters), num_clusters)
...
def lambda_handler(event, context):
	...
	if (worker_index == 0):
		...
		put_object(avg_cent_bucket, "initial", cent.tobytes())
	else:
	...
	for epoch in range(num_epochs):
		...
		if worker_index == 0:
			compute_average_centroids(avg_cent_bucket, worker_cent_bucket, num_workers, shape, epoch);
